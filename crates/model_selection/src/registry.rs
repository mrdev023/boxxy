...
pub trait AiProvider {
    ...
    fn create_model_provider(&self, model_idx: u32, model_name: Option<String>, thinking_idx: Option<u32>) -> ModelProvider;
    ...
    fn sync_ui(&self, provider: &ModelProvider, model_dropdown: &gtk::DropDown, thinking_dropdown: &gtk::DropDown, model_list: &gtk::StringList, thinking_list: &gtk::StringList);
}
...
impl AiProvider for GeminiProviderImpl {
    ...
    fn get_models(&self) -> Vec<String> {
        GeminiModel::all().into_iter().map(|m| m.to_string()).collect()
    }
    ...
    fn get_thinking_levels(&self, model_idx: u32) -> Vec<String> {
        let am = GeminiModel::all();
        if let Some(m) = am.get(model_idx as usize) {
            return m.available_thinking_levels().into_iter().map(|l| l.to_string()).collect();
        }
        vec![]
    }
    fn create_model_provider(&self, model_idx: u32, _name: Option<String>, thinking_idx: Option<u32>) -> ModelProvider {
        let am = GeminiModel::all();
        let model = am.get(model_idx as usize).cloned().unwrap_or(GeminiModel::Flash);
        let levels = model.available_thinking_levels();
        let thinking = thinking_idx.and_then(|idx| levels.get(idx as usize)).cloned();
        ModelProvider::Gemini(model, thinking)
    }
    fn sync_ui(&self, provider: &ModelProvider, model_dropdown: &gtk::DropDown, thinking_dropdown: &gtk::DropDown, _model_list: &gtk::StringList, thinking_list: &gtk::StringList) {
...
impl AiProvider for AnthropicProviderImpl {
    ...
    fn get_models(&self) -> Vec<String> {
        AnthropicModel::all().into_iter().map(|m| m.to_string()).collect()
    }
    fn create_model_provider(&self, model_idx: u32, _name: Option<String>, _thinking: Option<u32>) -> ModelProvider {
        let am = AnthropicModel::all();
        let model = am.get(model_idx as usize).cloned().unwrap_or(AnthropicModel::ClaudeSonnet);
        ModelProvider::Anthropic(model)
    }
    fn sync_ui(&self, provider: &ModelProvider, model_dropdown: &gtk::DropDown, _thinking_dropdown: &gtk::DropDown, _model_list: &gtk::StringList, _thinking_list: &gtk::StringList) {
...
impl AiProvider for OllamaProviderImpl {
    ...
    fn get_models(&self) -> Vec<String> { vec!["Loading...".to_string()] }
    fn create_model_provider(&self, _idx: u32, name: Option<String>, _think: Option<u32>) -> ModelProvider {
        ModelProvider::Ollama(name.unwrap_or_default())
    }
    fn sync_ui(&self, provider: &ModelProvider, model_dropdown: &gtk::DropDown, _thinking_dropdown: &gtk::DropDown, model_list: &gtk::StringList, _thinking_list: &gtk::StringList) {
        if let ModelProvider::Ollama(m) = provider {
            let mut found_pos = None;
            for i in 0..model_list.n_items() {
                if let Some(item) = model_list.item(i).and_then(|o| o.downcast::<gtk::StringObject>().ok()) {
