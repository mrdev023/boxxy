...
- **Persistent-Shells Dispatch (Experimental)**: `PaneInner::drop` reads the live `pty_persistence` setting. When off, the pane sends `signal_process_group(pid, SIGTERM)` — the shell and its children (mpv, etc.) are killed. When on, it calls `set_persistence(pid, true)` followed by `detach(pid)` so the daemon takes over and the shell keeps running in the background. The setting is read at close-time so toggling takes effect on the very next close, not just on panes opened after the toggle.
